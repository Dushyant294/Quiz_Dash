...
async function handleCSVUpload({ questions, fileName, fileUrl, categoryId, subject, topic, microTopic, userId }) {
  ...
  for (const q of questions) {
    ...
    if (qCatInfo && !catId) {
       ...
       const newCat = await CategoryModel.createCategory(cleanName);
    ...
    let subId = null;
    if (catId && qSubInfo) {
      if (!cache.subjects[catId]) cache.subjects[catId] = await CategoryModel.getSubjects(catId);
      subId = normalizeAndMatch(qSubInfo, cache.subjects[catId], 'name', 'subject_id');
      if (!subId) {
        const cleanName = titleCase(qSubInfo.trim());
        const newSub = await CategoryModel.createSubject(catId, cleanName);
    ...
    let topId = null;
    if (subId && qTopInfo) {
      if (!cache.topics[subId]) cache.topics[subId] = await CategoryModel.getTopics(subId);
      topId = normalizeAndMatch(qTopInfo, cache.topics[subId], 'name', 'topic_id');
      if (!topId) {
        const cleanName = titleCase(qTopInfo.trim());
        const newTop = await CategoryModel.createTopic(subId, cleanName);
    ...
    let mTopId = null;
    if (topId && qMicroInfo) {
      if (!cache.microTopics[topId]) cache.microTopics[topId] = await CategoryModel.getMicroTopics(topId);
      mTopId = normalizeAndMatch(qMicroInfo, cache.microTopics[topId], 'name', 'micro_topic_id');
      if (!mTopId) {
        const cleanName = titleCase(qMicroInfo.trim());
        const newMTop = await CategoryModel.createMicroTopic(topId, cleanName);
