...
function News() {
    ...
    const filters = ['All Updates', 'NEW FEATURE', 'PERFORMANCE', 'UI IMPROVEMENT'];
                ...
                {filters.map(filter => (
                    <span
                        ...
                        className={`rounded-full px-5 py-1 text-xs font-bold cursor-pointer transition tracking-wide ${
                            activeFilter === filter
                                ? 'border-2 border-white text-white bg-white/10 shadow-lg'
                                : 'border border-gray-400 text-gray-300 font-semibold hover:bg-white/10'
                        }`}
                    >
                        {filter}
                    </span>
